function openPage(t,e){for(;t!=window.body&&!(t instanceof IOSTab);)t=t.parentElement;t instanceof IOSTab&&t.showNewPage(e)}function loadSVG(t,e){if(t.trim().startsWith("<svg"))return e(t);fetch(t).then(t=>t.text()).then(e)}function createSVGElement(t){var e=document.createElement("template");return e.innerHTML=t.trim(),e.content.firstChild}function mirrorEvents(e,s,t){t.forEach(t=>{e.addEventListener(t,t=>s.dispatchEvent(new t.constructor(t.type,t)))})}class IOSApp extends HTMLElement{static _cssCallbacks=[];static notifyCSS(){var t=[...document.styleSheets].find(t=>[...t.cssRules].find(t=>t.cssText.includes("i-app-detection-label"))),t=(IOSApp.css=new CSSStyleSheet,IOSApp.css.replace([...t.cssRules].map(t=>t.cssText).join("\n")),IOSApp._cssCallbacks);IOSApp._cssCallbacks=void 0,t.forEach(t=>t())}loadStyleSheet(t){IOSApp._cssCallbacks?IOSApp._cssCallbacks.push(t):t()}addStyleToShadow(e){var s=[IOSApp.css];for(let t=0;t<e.adoptedStyleSheets.length;t++)s.push(e.adoptedStyleSheets[t]);e.adoptedStyleSheets=s}connectedCallback(){this.initialized||(this.initialized=!0,this.loadStyleSheet(()=>{this.attachShadow({mode:"open"}),this.addStyleToShadow(this.shadowRoot),this.shadowRoot.innerHTML+=`
				<i-tabbar></i-tabbar>
				<slot></slot>
			`,this.bottomMenu=this.shadowRoot.querySelector("i-tabbar"),this.bottomMenu.bindApp(this),new ResizeObserver(t=>{this.getBoundingClientRect();this.classList.toggle("large-screen",600<=t[0].contentRect.width)}).observe(this),new MutationObserver(t=>t.forEach(t=>t.addedNodes.forEach(t=>{t instanceof IOSTab&&this._processBindTab(t)}))).observe(this,{childList:!0}),[...this.children].forEach(t=>{t instanceof IOSTab&&this._processBindTab(t)}),this.hasAttribute("manifest")&&this._loadFromManifest()}))}_processBindTab(t){t._bindApp(this),!t.hasAttribute("selected")&&1!=this.tabsCount||this.selectTab(t.id)}_loadFromManifest(){fetch(this.getAttribute("manifest")).then(t=>t.json()).then(t=>{t.tabs.forEach(t=>{var e=document.createElement("i-tab");t.selected&&e.setAttribute("selected",""),t.id&&(e.id=t.id),t.name&&(e.name=t.name),t.page&&(e.page=t.page),t.icon&&(e.icon=t.icon),this.appendChild(e)})})}selectTab(t){t=this.querySelector("#"+t);this.selectedTab?.deselected(),(this.selectedTab=t).selected()}_animateTransition(s,i,a=t=>t){let n=new IOSAnimationId;var o=bezier(.2,.8,.2,1),r=Date.now(),c=this;this._transitionStarted(n,s),function t(){var e=(Date.now()-r)/i;1<=e?(c._processTransitionFrame(s,a(1)),c._transitionCompleted(n,s,1==a(1))):(c._processTransitionFrame(n,s,a(o(e))),requestAnimationFrame(t))}()}_transitionStarted(t,e){this._animationId=t,this.dispatchEvent(new CustomEvent("transition-started",{detail:{page:e}}))}_transitionCompleted(t,e,s){this._animationId==t&&this.dispatchEvent(new CustomEvent("transition-completed",{detail:{page:e,isEnd:s}}))}_processTransitionFrame(t,e,s){this._animationId==t&&(e.style.transform=`translateX(${100*(1-s)}%)`,e.prevPage.style.transform=`translateX(${-30*s}%)`,this.dispatchEvent(new CustomEvent("transition",{detail:{page:e,percent:s}})))}}class IOSAnimationId{}window.customElements.define("i-app",IOSApp);var NEWTON_ITERATIONS=4,NEWTON_MIN_SLOPE=.001,SUBDIVISION_PRECISION=1e-7,SUBDIVISION_MAX_ITERATIONS=10,kSplineTableSize=11,kSampleStepSize=1/(kSplineTableSize-1),float32ArraySupported="function"==typeof Float32Array;function A(e,r){return 1-3*r+3*e}function B(e,r){return 3*r-6*e}function C(e){return 3*e}function calcBezier(e,r,n){return((A(r,n)*e+B(r,n))*e+C(r))*e}function getSlope(e,r,n){return 3*A(r,n)*e*e+2*B(r,n)*e+C(r)}function binarySubdivide(e,r,n,t,i){for(var S,a,o=0;0<(S=calcBezier(a=r+(n-r)/2,t,i)-e)?n=a:r=a,Math.abs(S)>SUBDIVISION_PRECISION&&++o<SUBDIVISION_MAX_ITERATIONS;);return a}function newtonRaphsonIterate(e,r,n,t){for(var i=0;i<NEWTON_ITERATIONS;++i){var S=getSlope(r,n,t);if(0===S)return r;r-=(calcBezier(r,n,t)-e)/S}return r}function LinearEasing(e){return e}function bezier(a,r,o,n){if(!(0<=a&&a<=1&&0<=o&&o<=1))throw new Error("bezier x values must be in [0, 1] range");if(a===r&&o===n)return LinearEasing;for(var u=new(float32ArraySupported?Float32Array:Array)(kSplineTableSize),e=0;e<kSplineTableSize;++e)u[e]=calcBezier(e*kSampleStepSize,a,o);return function(e){return 0===e||1===e?e:calcBezier(function(e){for(var r=0,n=1,t=kSplineTableSize-1;n!==t&&u[n]<=e;++n)r+=kSampleStepSize;var i=r+(e-u[--n])/(u[n+1]-u[n])*kSampleStepSize,S=getSlope(i,a,o);return NEWTON_MIN_SLOPE<=S?newtonRaphsonIterate(e,i,a,o):0===S?i:binarySubdivide(e,r,r+kSampleStepSize,a,o)}(e),r,n)}}class IOSPage extends HTMLElement{_bindTab(t){this.tab=t,this.app=t.app,this.title=this.getAttribute("title")||"Untitled",this.titlebarType=this.getAttribute("titlebar")||"default",this.inverted="true"===this.getAttribute("inverted")||!1,this.path=this.getAttribute("path")||".",this.src=this.getAttribute("src")||void 0,this.inverted&&this.classList.add("inverted"),this.attachShadow({mode:"open"}),this.app.addStyleToShadow(this.shadowRoot),this.shadowRoot.innerHTML+=`
	    	<page-shadow>
	...
	`;_bindTab(t){this.tab=t;this.animation={},this.innerHTML=`
		...
		`}}window.customElements.define("search-box",SearchBox);class TableView extends HTMLElement{connectedCallback(){this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=`
	    	<style>
	    		#title {
	    			display: none;
	    			height: 30px;
		...
		`}}class SwitchCellDecorator{applyCell(t){t.innerHTML+='<ios-checkbox slot="detail"></ios-checkbox>'}}class RadioCellDecorator{static groups=new Object;applyCell(e){e.clickable=!0,e.select=()=>{RadioCellDecorator.groups[e.groupName]?.shadowItem.classList.remove("selected"),(RadioCellDecorator.groups[e.groupName]=e).shadowItem.classList.add("selected"),e.dispatchEvent(new CustomEvent("change",{detail:{target:e}}))},e.groupName=e.getAttribute("group"),e.shadowItem=e.shadowRoot.querySelector("#item"),e.hasAttribute("selected")&&e.select(),e.addEventListener("click",t=>{e.select()}),e.shadowRoot.querySelector("#content").innerHTML+=`
			<style>
				#check {
					display: none;
					color: var(--i-accent);
